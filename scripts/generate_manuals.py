...
'micro-mood-journal': {
 ...
 'steps':['가장 남은 순간, 지친 이유, 고마운 것 등 오늘의 질문을 선택합니다.','세 개 입력란에 오늘을 정리하는 문장을 한 줄씩 씁니다.','현재 감정 점수와 무드 아이콘을 선택합니다.','오늘 무드 기록하기를 눌러 저장합니다.','최근 무드 로그에서 지난 기록을 돌아보고 불필요한 기록은 삭제합니다.'],
 'output':'날짜별 세 문장 일기, 감정 점수와 무드 로그',
 'cautions':['로그 삭제는 되돌리기 어려울 수 있으므로 신중히 사용하세요.','브라우저 저장 데이터 삭제 또는 기기 변경 시 기록이 사라질 수 있습니다.']},
'lunch-roulette': {
 ...
 'cautions':['주변 맛집 정보와 영업시간은 실제 지도·매장 정보로 확인하세요.','알레르기·종교·건강상의 제외 음식은 룰렛 전에 반드시 반영하세요.']},
'mastermind-page': {
 'purpose':'리더·창업자 대상 Mastermind 프로그램의 대상, 방식, 코치와 FAQ를 탐색합니다.',
 'steps':['첫 화면에서 프로그램이 해결하려는 문제와 대상 설명을 읽습니다.','Who This Is For에서 본인의 역할과 적합성을 확인합니다.','How This Works에서 두 Mastermind 운영 구조를 살펴봅니다.','Coach와 Monthly VIP Guests 섹션에서 진행 방식과 네트워크를 확인합니다.','FAQ 질문을 눌러 일정·참여 방식·장소·비용과 결제 정보를 펼쳐봅니다.'],
 'output':'Mastermind 프로그램의 대상, 운영 방식, 코치·게스트 및 FAQ 정보',
 'cautions':['이 페이지는 프로그램 안내용이며 신청·결제 조건은 최신 공식 안내를 확인하세요.','영문 콘텐츠이므로 중요한 비용과 일정 문구를 정확히 확인하세요.']},
'namecard-memo-indexer': {
 ...
 'cautions':['OCR 연락처는 명함 원본과 대조해 오탈자를 수정하세요.','명함과 연락처 개인정보는 당사자 동의와 관련 법규에 맞게 관리하세요.']},
...
'moa-ai-bookmark-manager': {
 'purpose':'YouTube·아티클 링크를 분석해 제목·요약·태그·컬렉션으로 정리합니다.',
 'steps':['링크 저장을 눌러 YouTube 또는 아티클 URL을 입력합니다.','AI로 분석을 눌러 제목, 요약과 태그를 생성합니다.','분석 내용을 확인하고 필요하면 다시 분석합니다.','컬렉션을 선택한 뒤 보관함에 저장합니다.','검색·카테고리·콘텐츠 유형·즐겨찾기 필터로 저장한 자료를 다시 찾습니다.'],
 'output':'링크 원문 정보, AI 요약·태그, 컬렉션별 북마크 카드',
 'cautions':['자막이나 원문 접근이 제한된 링크는 분석 결과가 부족할 수 있습니다.','AI 요약은 원문을 대체하지 않으므로 중요한 정보는 원문 열기로 확인하세요.']},
'downdoc': {
 ...
 'output':'파일 유형·중복·이름 상태 진단과 정리 전후 미리보기',
 'cautions':['웹 배포판은 사용자의 실제 PC 폴더를 직접 변경하지 않는 체험 모드일 수 있습니다.','실제 파일 정리 전에는 중요한 파일을 백업하고 계획을 항목별로 확인하세요.']}
}
