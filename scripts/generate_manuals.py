...
MANUALS = {
'camping-checklist': {
 'purpose':'캠핑 유형, 숙박 기간과 인원수에 맞는 준비물을 자동 추천받아 빠뜨림 없이 출발을 준비합니다.',
 'steps':['백패킹·오토캠핑·차박캠핑·카라반·모터홈 중 이번 캠핑 유형을 선택합니다.','숙박 수와 함께 준비물을 사용할 전체 인원수를 설정합니다.','다음을 누르고 필요한 준비 카테고리를 켜거나 전체 선택을 사용합니다.','맞춤 체크리스트 만들기를 누르면 추천 품목이 모두 기본 선택된 상태로 생성됩니다.','필요 없는 물품의 체크를 해제하고, 빠진 물품은 각 카테고리 목록 아래의 준비물 추가 버튼으로 바로 보충합니다. 품목을 누르면 수량·중요도·메모도 수정할 수 있습니다.','선택한 항목만 PDF 저장 또는 XLSX 저장으로 내려받고, 체크리스트 저장으로 현재 기기에 보관합니다.'],
 'output':'캠핑 유형·일정·인원에 맞춘 준비물 목록, 품목별 수량과 중요도, 선택 품목 수, 개인 메모 및 선택 항목 PDF·XLSX 파일',
 'cautions':['저장 데이터는 현재 브라우저와 기기의 로컬 저장소에 보관되므로 다른 기기와 자동 동기화되지 않습니다.','선택 품목까지 포함하면 목록이 크게 늘어날 수 있으므로 실제 캠핑장 시설과 반입 규정을 기준으로 조정하세요.']},
...
'gemini-notebook-video-style-picker': {
 'purpose':'Gemini Notebook 동영상에 적용할 맞춤 시각 스타일을 비교하고 두 입력란에 사용할 프롬프트를 복사합니다.',
 'steps':['정보 전달형과 이미지 중심형으로 구분된 20개 스타일의 미리보기를 살펴봅니다.','영상 주제와 목적에 맞는 스타일 카드를 선택합니다.','스타일의 권장 용도와 주의사항을 확인합니다.','1번 맞춤 시각적 스타일 설명을 복사해 Notebook 팝업의 해당 입력란에 붙여넣습니다.','2번 AI 호스트 지시문의 대괄호 부분을 영상 주제로 바꾼 뒤 복사해 해당 입력란에 붙여넣습니다.'],
 'output':'선택한 테마의 영어 시각 스타일 프롬프트와 한국어 AI 호스트 집중 지시문',
 'cautions':['팝업의 형식은 설명 동영상, 언어는 한국어, 시각적 스타일은 맞춤으로 설정하세요.','결과가 스타일 지시에서 벗어나면 소스를 5~6개로 줄이거나 영상을 3~4분 단위로 나눠 다시 생성하세요.']},
'lotto-number-picker': {
 'purpose':'1부터 45까지 중복 없는 로또 번호 6개를 한 번에 다섯 조합으로 무작위 추천합니다.',
 'steps':['번호 추천받기 버튼을 누릅니다.','1조합부터 5조합까지 생성되는 번호를 확인합니다.','각 조합의 여섯 번호가 오름차순으로 정리되었는지 살펴봅니다.','노란색·파란색·빨간색·회색·초록색 공으로 번호 구간을 구분합니다.','새 번호가 필요하면 다시 추천받기를 눌러 다섯 조합을 새로 만듭니다.'],
 'output':'1~45 범위에서 조합마다 중복 없이 생성된 로또 번호 6개씩 총 5개 조합',
 'cautions':['추천 번호는 암호학적 난수로 무작위 생성되며 당첨을 보장하지 않습니다.','재미와 참고 목적으로 이용하고 과도한 구매는 피하세요.']},
'memory-companion': {
